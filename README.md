...
> **Tips on Reference Audio & Voice Cloning Quality**
>
> - **Optimal Duration is 5–15 seconds (Not "the longer the better")**:
>   - *Context Window & Speed*: Zero-shot TTS encodes reference audio into continuous acoustic tokens. Overly long audio consumes valuable attention context, increases VRAM usage, and significantly slows down inference.
>   - *Attention Drift & Hallucinations*: Overly long prompt tokens (>25s) cause attention distraction, leading to unnatural pauses, repeated words, pronunciation glitches, or language bleeding.
>   - *Acoustic Noise Accumulation*: A 10-second clip can easily be kept pristine. Longer audio inevitably contains breath intakes, mouth clicks, room reverb, background noise, or fluctuating cadence—the model will memorize these artifacts as part of the speaker's intrinsic timbre!
>   - *ASR Window Limits*: Standard speech recognition (Whisper) caps at 30 seconds (3,000 mel frames) per chunk. Keeping references under 20s ensures zero alignment boundary errors.
>   - *Key Rule*: **Audio quality, purity, and timbre representativeness matter far more than audio length.**
> - **Clean Dry Voice (干声)**: Use audio without background music (BGM), heavy reverb, or ambient noise.
> - **Strict Text Alignment**: The reference text must match the reference speech word-for-word (including filler words).
> - **Accent & Language**: For native pronunciation, use reference audio in the same language as the target speech. In cross-lingual cloning, the output naturally inherits the accent of the reference audio.
> - **Number Normalization**: Pass `normalize_text=True` to expand Arabic numerals into written words (requires `pip install "omnivoice[tn]"`).
>
