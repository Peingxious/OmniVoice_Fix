...
class OmniVoice(PreTrainedModel):
    ...
    def _prepare_inference_inputs(
        ...
        # Defensive check: Ensure ref_text does not vastly exceed ref_audio_tokens
        # To avoid the diffusion model uttering trailing phantom ref_text in target speech
        if ref_audio_tokens is not None and ref_text:
            num_ref_frames = ref_audio_tokens.size(-1)
            audio_sec = num_ref_frames / 12.5
            has_cjk = bool(_ZH_RE.search(ref_text))
            max_allowed_chars = int(audio_sec * (4.5 if has_cjk else 16.0)) + 10
            if len(ref_text) > max_allowed_chars:
                candidates = [
                    idx for idx, ch in enumerate(ref_text[:max_allowed_chars])
                    if ch in "。！？.!?;；\n"
                ]
                if candidates:
                    ref_text = ref_text[:max(candidates) + 1]
                else:
                    ref_text = ref_text[:max_allowed_chars]
                logger.info(
                    f"Aligned ref_text to {len(ref_text)} chars to match ref_audio duration ({audio_sec:.1f}s)"
                )
